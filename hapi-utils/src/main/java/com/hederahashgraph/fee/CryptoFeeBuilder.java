...
public class CryptoFeeBuilder extends FeeBuilder {
	...
	public FeeData getCryptoUpdateTxFeeMatrices(TransactionBody txBody, SigValueObj sigValObj,
		...
		boolean isKeyPresent = crUpdateTxBody.hasKey();

		if (hasExpiration(expirationTimeStamp, crUpdateTxBody)) {
			if (expiresAfter(expirationTimeStamp, crUpdateTxBody)) {
				rbs = rbs + (isKeyPresent ? getRbs(crUpdateTxBody, crUpdateTxBody.getKey(), expirationTimeStamp) :
						getRbs(crUpdateTxBody, existingKey, expirationTimeStamp));
			} else {
				if (isKeyPresent) {
	...
	private long getRbs(CryptoUpdateTransactionBody crUpdateTxBody, Key key,
			Timestamp expirationTimeStamp) {
		return (BASIC_ACCOUNT_SIZE + getAccountKeyStorageSize(key))
				* (crUpdateTxBody.getExpirationTime().getSeconds()
				- expirationTimeStamp.getSeconds());
	}


	private boolean expiresAfter(Timestamp expirationTimeStamp, CryptoUpdateTransactionBody crUpdateTxBody) {
		return crUpdateTxBody.getExpirationTime().getSeconds() > expirationTimeStamp.getSeconds();
	}
	 ...
	 */
	public FeeData getCryptoAccountRecordsQueryFeeMatrices(List<TransactionRecord> transRecord,
			ResponseType responseType) {
	...
	private int getAccountInfoSize(Key accountKey, List<LiveHash> liveHashes) {
		 ...
		 * autoRenewPeriod - LONG_SIZE repeated LiveHash claims - calculated value AccountID accountID -
	...
	private boolean hasExpiration(Timestamp expirationTimeStamp, CryptoUpdateTransactionBody crUpdateTxBody) {
		return expirationTimeStamp != null && expirationTimeStamp.getSeconds() > 0 && crUpdateTxBody.hasExpirationTime();
	}
}
