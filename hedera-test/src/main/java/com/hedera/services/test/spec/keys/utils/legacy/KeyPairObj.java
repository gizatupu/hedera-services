package com.hedera.services.test.spec.keys.utils.legacy;
...
public class KeyPairObj implements Serializable {
	...
	public PublicKey getPublicKey_64() {
		...
		try {
			pubKeyBytes = Hex.decodeHex(publicKey);
		} catch(Exception e) {
			e.printStackTrace();
		}
		EdDSAParameterSpec spec = EdDSANamedCurveTable.getByName(EdDSANamedCurveTable.ED_25519);
	...
	public String getPublicKeyAbyteStr() throws InvalidKeySpecException, DecoderException {
		return Hex.encodeHexString(((EdDSAPublicKey) getPublicKey()).getAbyte());
	}
