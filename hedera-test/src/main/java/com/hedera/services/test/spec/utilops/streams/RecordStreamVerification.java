package com.hedera.services.test.spec.utilops.streams;
...
import com.hedera.services.test.spec.utilops.UtilOp;
import com.hedera.services.test.spec.verification.NodeSignatureVerifier;
import com.hedera.services.test.spec.verification.RecordFileParser;
...
public class RecordStreamVerification extends UtilOp {
	...
	protected boolean submitOp(HapiApiSpec spec) throws Throwable {
		...
		Map<String, List<File>> sigFilesAvail = uniqRecordFiles
				.stream()
				.flatMap(rcd -> verifier.nodes()
						.stream()
						.map(account -> Path.of(recordsDirFor(account), rcd)))
				.collect(
						groupingBy(this::basename,
								mapping(
										(Path p) -> Path.of(p.toFile().getAbsolutePath() + "_sig").toFile(),
										filtering(File::exists, toList()))));
		checkOverallValidity(sigFilesAvail, verifier);
		Assert.assertTrue(
				"Not everything seemed good with the record streams, see logs above!",
				allGood);
		return false;
	}

	private void checkOverallValidity(Map<String, List<File>> sigFilesAvail, NodeSignatureVerifier verifier) {
		sigFilesAvail.entrySet()
				.stream()
				.filter(entry -> entry.getValue().size() > 0)
				.forEach(entry -> checkSigValidity(entry.getKey(), entry.getValue(), verifier));

		List<String> orderedRcdNames = sigFilesAvail.keySet()
				.stream()
		...
		for (String account : verifier.nodes()) {
			...
			List<RecordFileParser.RecordFile> recordFiles = orderedRcdNames
					.stream()
	...
	private void checkSigValidity(String record, List<File> sigs, NodeSignatureVerifier verifier) {
		...
		if (majority == null) {
			...
			setUnGood(
					"Only {} agreed on the hash of record file '{}'!",
					majority,
					record);
		} else {
			log.info(
